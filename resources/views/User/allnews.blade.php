                ...
                <section class="faq-section">
                    <section class="faq-section">
                        @forelse (\App\Models\News::all()->sortByDesc('updated_at') as $news)
                            @php
                                $isRead = $user->newsStatuses->contains('news_id', $news->id);
                            @endphp
                            ...
                            <details style="border: 2px solid {{ $isRead ? 'green' : 'red' }};">
                                <summary>{{ $news->title }} <span
                                        ...
                                        News</span> {!! $isRead == true? '<span style="color:green; font-size: .8rem; font-style: italic;">Read</span>' : '<span style="color:red; font-size: .8rem; font-style: italic;">UnRead</span>' !!}</summary>
