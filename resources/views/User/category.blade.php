...
<body>
    ...
    <div class="flex p-2 gap-4">
        ...
        <div class="p-4 flex-grow">
            <div class="mb-4 text-sm font-semibold text-orange-500 text-center border-2 p-2 border-gray-800">
                <p class="mt-0 mb-2 text-2xl">{{ $categoryName }}</p>
                Please select your desired sub-category for fair results. New vendors will have a fair chance of their
                products being displayed.
            </div>
            <div
                class="md:mb-14 lg:mb-14 p-4 flex-grow rounded-md shadow-md border border-gray-500 hover:border-8 m-2 hover:border-gray-700 hover:text-gray-700 active:border-gray-800 active:text-gray-800 transition-all duration-300">
                <div class="grid md:grid-cols-3 lg:grid-cols-3 sm:grid-cols-2 gap-4 mt-5 justify-center">
                    @forelse ($subCategories as $category)
                        <a href="/sub/category/{{ $category->created_at->timestamp }}/{{ $category->id }}"
                            class="p-2 rounded-md hover:border-8 hover:border-gray-700 active:border-gray-800 transition-all duration-300 bg-gray-900 cursor-pointer">
                            <span class="hover:text-orange-500">{{ $category->name }}</span><span
                                class="text-orange-500 font-medium">({{ \App\Models\Product::where('status', 'Active')->where('sub_category_id', $category->id)->count() }})</span>
                        </a>
                        @empty
                        <p>Empty</p>
                    @endforelse
