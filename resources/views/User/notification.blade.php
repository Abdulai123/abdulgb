...
<head>
    ...
    <title>Grand Bazaar | {{ $user->public_name }} > Notifications</title>
    ...
    <div class="main-div">
        <div class="notific-container">
            ...
            <table>
                ...
                <tbody>
                    <form action="/{{ $user->public_name }}/notifications/search" method="get" style="text-align: center">
                        <tr>
                            <td>
...
@endphp
            @forelse ($notifications as $notification)
                <div class="notification-container {{ $notification->is_read ? 'read' : '' }}">
                    ...
                    <img src="data:image/jpeg;base64,{{ $icon[$notification->notificationType->icon] }}"
                        class="icon-filter" alt="" width="30">
                    <div class="notification-content">
                        ...
                        <p class="notification-message" style="margin-top: 0px; font-size: .8rem">
                            ...
                            @if ($notification->notificationType->icon == 'order' || $notification->notificationType->icon == 'dispute')
                                <a href="/order/{{ $notification->order->created_at->timestamp ?? null }}/{{ $notification->option_id ?? null }}"
                                    style="font-size: 1rem; text-decoration:underline;">see order details here</a>
                                
                            @endif
                            @if ($notification->notificationType->icon == 'product')
                            @php
                                $product = \App\Models\Product::where('id', $notification->option_id)->first();
                            @endphp
                            <a href="/listing/{{ $product->created_at->timestamp ?? null }}/{{ $product->id ?? null }}"
                                style="font-size: 1rem; text-decoration:underline;">see the product here</a>
