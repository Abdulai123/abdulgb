    ...
    <div class="main-store-div">
        <div class="s-main-image">
            ...
            <div>
                ...
                <div class="div-p">
                    <p style="border-bottom: 3px dotted green">Last Seen: {{ \Carbon\Carbon::parse($new_store->user->last_seen)->diffForHumans() }}</p>
                    <p>Store Status: <span class="{{ $new_store->user->store_status }}">{{ $new_store->user->store_status }}</span></p>
                </div>

                <div class="div-p">
                    <p>Unauthorize Access: {{ $new_store->user->unauthorizes->count() }}</p>
                    <p>Reported Bugs: {{ $new_store->user->bugs->count() }}</p>
                </div>
                <div class="div-p">
                    <p>Reported Listings: {{ $new_store->user->reportedListings->count() }}</p>
                    <p>Reported Stores: {{ $new_store->user->reportedStores->count() }}</p>
                </div>
                <div class="div-p">
                    <p>Favorite Listings: {{ $new_store->user->favoriteListings->count() }}</p>
                    <p>Favorite Stores: {{ $new_store->user->favoriteStores->count() }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="main-store-div">
        ...
        @if ($new_store->sell_on != 'none')
            ...
            <div style="text-align: center">
                {{ $new_store->sell_on ?? "No selling on info provided...." }}
            </div>
        @endif
        <h3>Products Proof</h3>
        <div class="sub-pics" style="text-align: center">
            <img src="data:image/png;base64,{{ $upload_image[$new_store->proof1] }}" width="150">
            <img src="data:image/png;base64,{{ $upload_image[$new_store->proof2] }}" width="150">
            <img src="data:image/png;base64,{{ $upload_image[$new_store->proof3] }}" width="150">
        </div>
        <div class="bio">
