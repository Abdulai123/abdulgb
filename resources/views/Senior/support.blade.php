...
<table>
    ...
    <tbody>
        <form action="/senior/staff/{{ $user->public_name }}/show/supports/search" method="get" style="text-align: center">
            <tr>
                <td>
...
<table>
    ...
    <tbody>
        @php
            $supports = session('supports') ?? \App\Models\Support::orderByDesc('created_at')->paginate(50);
        @endphp
                ...
                <td>
                    <form action="" method="post">
                        ...
                        <input type="hidden" name="support_id" value="{{ Crypt::encrypt($ticket->id) }}">
                            <a href="/senior/staff/{{ $user->public_name }}/show/messages/{{ $ticket->conversation->created_at->timestamp }}/{{ $ticket->conversation_id }}"
