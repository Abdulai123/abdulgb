<div class="notific-container">
    ...
    <div style="text-align: center;" style="margin-bottom: 0px;">
        <a href="/whales/admin/{{ $user->public_name }}/start/new/message" class="input-listing">Start New Message</a><br><br>
    </div>
    ...
    <table>
        ...
        <tbody>
            <form action="/whales/admin/{{ $user->public_name }}/messages/show/search" method="get" style="text-align: center">
                @csrf
                <tr>
                    <td>
    ...
    @forelse ($AllParticipants->where('is_hidden', 0) as $userConversation)
        @foreach ($AllConversations->where('id', $userConversation->conversation_id) as $conversation)
            ...
            <a href="/whales/admin/{{ $user->public_name }}/show{{ $isDispute ? '/dispute/' . $conversation->dispute->created_at->timestamp . '/' . $conversation->dispute->id : '/messages/' . $conversation->created_at->timestamp . '/' . $conversation->id }}"
                ...
                <div class="notification-content">
                    <div style="display: flex;">
                        ...
                        <span class="notification-time">{{ $conversation->created_at->diffForHumans() }}</span>
                            <form
                                action="/whales/admin/{{ $user->public_name }}/conversation/archive/{{ $conversation->created_at->timestamp }}/{{ $conversation->id }}"
                                method="post">
                                @csrf
