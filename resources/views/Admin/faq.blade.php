<div class="notific-container">
    ...
    <div style="text-align: center; padding:0px; margin:0px;">
        <form action="/whales/admin/{{ $user->public_name }}/new/faq" method="post" style="margin: .3em;">
            @csrf
            @if (session('new_faq'))
            ...
            @elseif (session('faq'))
                @php
                    $faq = session('faq');
                @endphp
                <input type="text" name="equestion" class="form-input"
                    style="background-color: var(--white-background-color); color: var(--dark-color-text)"
                    placeholder="Write here your question that you gonna answer include ?..."
            ...
            @forelse (\App\Models\FAQ::all() as $faq)
                <details>
                    ...
                    <div style="display: flex; gap:2em;">
                        ...
                        <form action="/whales/admin/{{ $user->public_name }}/new/faq" method="post">
                                @csrf
