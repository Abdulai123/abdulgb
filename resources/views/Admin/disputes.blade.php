...
<table>
    ...
    <tbody>
        <form action="/whales/admin/{{ $user->public_name }}/show/disputes/search" method="get" style="text-align: center">
            <tr>
                <td>
...
<table>
    ...
    <tbody>
        @php
        $disputes = session('disputes') ?? \App\Models\Dispute::paginate(50);
    @endphp
        ...
        @forelse ($disputes as $dispute)
            ...
            <tr>
                ...
                <td>
                    <form action="/whales/admin/{{ $user->public_name }}/do/dispute/{{ $dispute->created_at->timestamp }}/{{ $dispute->id }}" method="post">
                        @csrf
                        @if (($dispute->mediator_id == null) || $dispute->mediator_id == $user->id)
                            <a href="/whales/admin/{{ $user->public_name }}/show/dispute/{{ $dispute->created_at->timestamp }}/{{ $dispute->id }}"
