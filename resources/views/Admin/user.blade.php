<div class="main-div" style="margin-top:0px">
    <div class="main-store-div">
            ...
            <div>
                ...
                <div class="div-p">
                    <p style="border-bottom: 3px dotted green">Last Seen:
                        {{ \Carbon\Carbon::parse($show_user->last_seen)->diffForHumans() }}</p>
        ...
        <form action="" method="post" style="margin-top: 1em;">
            ...
            @if (($show_user->status == 'active' || $show_user->status == 'vacation') && $show_user->role != 'admin')
                <button type="submit"
            ...
            @if ($show_user->role == 'user')
                <button type="submit"
                    style="font-size: 1rem; background-color: darkorange; color: #f1f1f1; cursor:pointer; padding: 5px; border: none; border-radius: .5rem;"
                    name="senior">Promote To Senior Mod</button>
                <button type="submit"
                    style="font-size: 1rem; background-color: orange; color: #f1f1f1; cursor:pointer; padding: 5px; border: none; border-radius: .5rem;"
                    name="junior">Promote To Junior Mod</button>
            @elseif($show_user->role == 'senior' || $show_user->role == 'junior')
                <button type="submit"
                    style="font-size: 1rem; background-color: red; color: #f1f1f1; cursor:pointer; padding: 5px; border: none; border-radius: .5rem;"
                    name="revoke_role">Revoke Role</button>
            @endif

            <button type="submit"
            style="font-size: 1rem; background-color: red; color: #f1f1f1; cursor:pointer; padding: 5px; border: none; border-radius: .5rem;"
            name="delete">Delete {{ $show_user->public_name }}</button>

            <button type="submit"
            style="font-size: 1rem; background-color: rgb(148, 35, 35); color: #f1f1f1; cursor:pointer; padding: 5px; border: none; border-radius: .5rem;"
            name="reset_password">Reset Password {{ $show_user->public_name }}</button>
        </form>
    </div>
</div>
                    ...
                    <tbody>
                        @forelse ($show_user->wallet->withdraw->sortByDesc('created_at') as $withdraw)
                            <tr>
                                ...
                                <td>{{ $withdraw->is_confirm == 1 ? 'Confirm' : 'pending' }}</td>
